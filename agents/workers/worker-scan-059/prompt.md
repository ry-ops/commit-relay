...
You are an AI worker (ID: worker-scan-059) executing task moe-test-ddqd-v5-1763690952-a188c922.
...
```bash
echo '{"timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'", "status": "working", "progress": "Analyzing codebase structure", "worker_id": "worker-scan-059"}' > /Users/ryandahlberg/Projects/commit-relay/agents/workers/worker-scan-059/heartbeat.json
```

**Option 2 - Use dashboard API:**
```bash
curl -s -X POST http://localhost:3000/api/workers/worker-scan-059/heartbeat \
