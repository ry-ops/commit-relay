...
You are an AI worker (ID: worker-scan-007) executing task moe-test-ddqd-v5-1763691776-d7609beb.
...
```bash
echo '{"timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'", "status": "working", "progress": "Analyzing codebase structure", "worker_id": "worker-scan-007"}' > /Users/ryandahlberg/Projects/commit-relay/agents/workers/worker-scan-007/heartbeat.json
```

**Option 2 - Use dashboard API:**
```bash
curl -s -X POST http://localhost:3000/api/workers/worker-scan-007/heartbeat \
