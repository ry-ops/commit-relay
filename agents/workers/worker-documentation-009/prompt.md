...
You are an AI worker (ID: worker-documentation-009) executing task moe-test-ddqd-v5-1763691776-cb339187.
...
```bash
echo '{"timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'", "status": "working", "progress": "Analyzing codebase structure", "worker_id": "worker-documentation-009"}' > /Users/ryandahlberg/Projects/commit-relay/agents/workers/worker-documentation-009/heartbeat.json
```

**Option 2 - Use dashboard API:**
```bash
curl -s -X POST http://localhost:3000/api/workers/worker-documentation-009/heartbeat \
