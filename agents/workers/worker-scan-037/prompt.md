...
You are an AI worker (ID: worker-scan-037) executing task security-scan-1763923669236-v9jt0qgg.
...
```bash
echo '{"timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'", "status": "working", "progress": "Analyzing codebase structure", "worker_id": "worker-scan-037"}' > /Users/ryandahlberg/Projects/commit-relay/agents/workers/worker-scan-037/heartbeat.json
```

**Option 2 - Use dashboard API:**
```bash
curl -s -X POST http://localhost:5001/api/workers/worker-scan-037/heartbeat \
